...
export default function ProfessionalReviewBanner({ descriptionKey = "review.description" }: ProfessionalReviewBannerProps) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50/60 dark:bg-emerald-950/20 dark:border-emerald-800 p-3 sm:p-4">
      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center flex-shrink-0">
        <UserCheck className="h-4 w-4 sm:h-[18px] sm:w-[18px] text-emerald-700 dark:text-emerald-400" />
      </div>
      <div className="min-w-0">
        <p className="text-xs sm:text-sm font-semibold text-emerald-800 dark:text-emerald-300">
          {T("review.title")}
        </p>
        <p className="text-[11px] sm:text-xs text-emerald-700/80 dark:text-emerald-400/80 mt-0.5 leading-relaxed">
          {T(descriptionKey)}
        </p>
      </div>
    </div>
