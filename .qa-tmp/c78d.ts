...
const COOKIE = readFileSync(".qa-tmp/cookie.txt", "utf8").trim();
const diary = (d: Date) =>
...
async function post(iso: string, note: string) {
  ...
  return "-> HTTP " + res.status + (res.status === 200 ? "" : " " + txt.slice(0, 160));
}
...
(async () => {
  ...
  for (const [dateStr, label, expected] of cases) {
    ...
    console.log("  --- BANCO (gravado pela rota POST /api/appointments) ---");
