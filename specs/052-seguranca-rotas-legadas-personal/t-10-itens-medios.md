...
**Status:** concluído (QA aprovado `qa/report-t-10.md` + code review aplicado)
**Depende de:** nenhuma
...
| 5 | `app/api/version/update/route.ts` | sem auth, sob `/api/version` pública → anônimo força reload em todos | **fora do escopo desta branch** (não é do personal/aluno): revertido em 18/09, a pedido do Bruno; fica como alerta para a frente de plataforma |
...
## Critérios de aceite
- [x] Checkout aberto 2x (duas chamadas seguidas) → a 1ª sessão é expirada ou a 2ª recusada. Nunca duas sessões abertas (teste com Stripe mockado/test-mode quando o Connect for ativado; até lá, teste unitário da lógica).
- [x] Webhook: `subscription.updated` com status `active` para assinatura CANCELLED → continua CANCELLED; status `paused` → não vira ACTIVE (teste unitário com payload assinado).
- [x] Cancelamento com erro da Stripe → status inalterado + erro na resposta.
- [x] `notifications/trigger` como ADMIN → 403; sem `CRON_SECRET` no env → rota recusa.
- [x] Mensagem do aluno com `<img src=x onerror=alert(1)>` → e-mail ao staff com o texto escapado.
- [x] Leaderboard sem `studentId` de outros; challenge ARCHIVED → 404 para aluno.
- [x] `performedAt: "abc"` → 400.
- [x] THERAPIST → reembolso/onboarding Connect → 403; ADMIN → funciona.
