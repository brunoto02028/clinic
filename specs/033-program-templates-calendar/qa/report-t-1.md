...
- **O1 (não-bloqueante, padrão já documentado no projeto).** "Sem sessão" responde **307 redirect** (middleware), não `401 {"error":"Unauthorized"}` do handler — mesmo comportamento de todo `/api/admin/**` hoje (já registrado em `specs/032-ai-workout-builder/qa/report-t-1-a-t-3.md` e `specs/20-.../qa/report-t-4.md`). Não é regressão desta tarefa; só nota pra quem escrever testes da T-6.
- Nenhum outro achado.
