// One-off script: import Tier 3 study-centre .md articles from
// BPR_Content_Tier3/Articles_markdown into Prisma, with pt-BR translations.
// All 5 are brand new articles (no existing slugs to update).
//
// Run with: node scripts/import-study-centre-articles-tier3.js
...
const ARTICLES_DIR = path.join(__dirname, '..', 'BPR_Content_Tier3', 'Articles_markdown');

const TAG_MAP = {
  'hip-osteoarthritis': ['Hip', 'Osteoarthritis'],
  'cervical-radiculopathy': ['Neck'],
  'tendinopathy-explained': ['Explained'],
  'persistent-pain-explained': ['Explained'],
  'return-to-running-after-injury': ['Explained'],
};
...
async function main() {
  ...
  for (const file of files) {
    ...
    const existing = await withRetry(() => prisma.article.findUnique({ where: { slug } }));
    if (existing) {
