// One-off script: import Tier 2 study-centre .md articles from
// BPR_Content_Tier2/Articles_markdown into Prisma, with pt-BR translations.
//
// Two of these files (GTPS, carpal tunnel) are upgraded versions of articles
// already created in an earlier session — for those we UPDATE the existing
// article in place (same slug/URL, same imageUrl, same published status),
// so we don't end up with two articles about the same topic.
// The remaining three (shin splints, tennis elbow, cervicogenic headache)
// are brand new and get created as drafts.
//
// Run with: node scripts/import-study-centre-articles-tier2.js
...
const ARTICLES_DIR = path.join(__dirname, '..', 'BPR_Content_Tier2', 'Articles_markdown');

// Maps the .md frontmatter slug -> the slug of an already-existing article
// this file should UPDATE instead of creating a new one.
const UPDATE_EXISTING_SLUG = {
  'greater-trochanteric-pain-gtps': 'greater-trochanteric-pain-syndrome-gtps',
  'carpal-tunnel-syndrome': 'carpal-tunnel-syndrome-numb-hand-at-night',
};

const TAG_MAP = {
  'shin-splints': ['Shin & Calf'],
...
async function main() {
  ...
  for (const file of files) {
    ...
    const tags = TAG_MAP[fileSlug] || ['Injury Prevention'];

    const targetSlug = UPDATE_EXISTING_SLUG[fileSlug] || fileSlug;
    const existing = await withRetry(() => prisma.article.findUnique({ where: { slug: targetSlug } }), 6, 3000);

    console.log(`\n=== ${fileSlug} ${existing ? `(updating existing: ${targetSlug})` : '(new)'} ===`);
    ...
    if (existing) {
      ...
      console.log(`OK — updated: ${targetSlug}`);
    } else {
      ...
      console.log(`OK — created: ${targetSlug}`);
    }
  }
