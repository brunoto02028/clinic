    ...
    <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
            <Card className="border-primary/30 shadow-lg">
              <CardContent className="p-4 space-y-4">
                ...
                {cart.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">{isPt ? "Seu carrinho está vazio" : "Your cart is empty"}</p>
                ) : (
                  <>
                    {/* Cart Items */}
                    <div className="space-y-2">
                      {cart.map((item) => (
                        <div key={item.product.id} className="flex items-center gap-3 bg-muted/30 rounded-lg p-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">£{item.product.price.toFixed(2)} {isPt ? "cada" : "each"}</p>
                          </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">{isPt ? "Nenhum produto disponível nesta categoria ainda" : "No products available in this category yet"}</p>
          <p className="text-xs text-muted-foreground mt-1">{isPt ? "Volte em breve — novos itens são adicionados regularmente" : "Check back soon — new items are added regularly"}</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <Card className="card-hover h-full flex flex-col overflow-hidden relative">
                  ...
                  {/* Image */}
                  {product.imageUrl ? (
                    <div className="h-36 bg-muted/50 overflow-hidden">
                      <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover" />
                    </div>
                  ) : (
                    <div className="h-36 bg-muted/30 flex items-center justify-center">
                      <CatIcon className="h-12 w-12 text-muted-foreground" />
                    </div>
                  )}
