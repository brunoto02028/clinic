    ...
    <div className="max-w-2xl mx-auto space-y-6 px-4 py-6">
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-right">
              <p className="text-lg font-bold text-ba1-warn">{totalXp} XP</p>
      ...
      {achievements.length === 0 ? (
        ...
        <>
          ...
          {unlocked.length > 0 && (
            <div>
              <h2 className="text-sm font-semibold text-ba1-ok mb-3 flex items-center gap-2">
                <Trophy className="h-4 w-4" /> {isPt ? `Desbloqueadas (${unlocked.length})` : `Unlocked (${unlocked.length})`}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {unlocked.map((a) => (
                  <Card key={a.id} className="border-ba1-ok/20 bg-ba1-ok/10 overflow-hidden">
