      ...
      <div className="space-y-6 max-w-3xl mx-auto">
        ...
        <div>
          ...
          {viewingAssignment?.note && (
            <div className="mt-3 p-3 bg-ba1-health/10 border border-ba1-health/20 rounded-lg text-sm">
              <p className="font-medium text-ba1-health flex items-center gap-1"><User className="h-3.5 w-3.5" /> {isPt ? `Nota de ${viewingAssignment.assignedBy?.firstName}:` : `Note from ${viewingAssignment.assignedBy?.firstName}:`}</p>
              <p className="text-ba1-health/80 mt-1">{viewingAssignment.note}</p>
            </div>
          )}
        </div>
                  ...
                  {[1, 2, 3, 4, 5].map(n => (
                    <button
                      ...
                      className={`w-8 h-8 rounded-full border flex items-center justify-center text-sm transition-colors ${
                        rating >= n ? "bg-ba1-warn text-white border-ba1-warn" : "border-border text-muted-foreground"
                      }`}
                    >
                      {n}
                    </button>
        ...
        {prog?.status === "completed" && (
          <div className="p-4 bg-ba1-ok/10 border border-ba1-ok/20 rounded-xl text-center">
            <CheckCircle className="h-8 w-8 text-ba1-ok mx-auto" />
            <p className="font-semibold text-ba1-ok mt-2">{isPt ? "Concluído!" : "Completed!"}</p>
            <p className="text-sm text-ba1-ok/80">{isPt ? "Ótimo trabalho acompanhando seu plano de tratamento." : "Great job keeping up with your treatment plan."}</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Card>
          <CardContent className="pt-3 pb-2 text-center">
            <p className="text-2xl font-bold text-ba1-ok">{completedAssignments.length}</p>
            <p className="text-[10px] text-muted-foreground">{isPt ? "Concluídos" : "Completed"}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-3 pb-2 text-center">
            <p className="text-2xl font-bold text-ba1-health">{published.length}</p>
                        ...
                        <div className="aspect-video bg-muted/50 rounded-t-lg -mx-6 -mt-0 mb-3 overflow-hidden relative">
                          ...
                          {prog?.status === "completed" && (
                            <div className="absolute top-2 right-2">
                              <Badge className="text-[10px] bg-ba1-ok text-white"><CheckCircle className="h-2.5 w-2.5 mr-0.5" />{isPt ? "Feito" : "Done"}</Badge>
                            </div>
                          )}
                          {item.isFeatured && (
