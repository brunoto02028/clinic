...
function RehabAgentTab({ patientId, patientData }: { patientId: string; patientData: any }) {
  ...
  const [form, setForm] = useState({
    ...
    age: patientData?.profile?.dateOfBirth
      ? String(new Date().getFullYear() - new Date(patientData.profile.dateOfBirth).getFullYear())
      : "",
  ...
  const handleChat = async () => {
    ...
    setActivePlan((p: any) => ({ ...p, messages: [...(p.messages || []), { role: "user", content: msg, createdAt: new Date() }] }));
    try {
      ...
      const d = await r.json();
      if (!r.ok) throw new Error(d.error);
      setActivePlan((p: any) => ({ ...p, messages: [...(p.messages || []), { role: "assistant", content: d.reply, createdAt: new Date() }] }));
    } catch (e: any) { setError(e.message); }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Clinical Rehab Agent</h3>
          <Badge variant="outline" className="text-[9px] border-emerald-500/30 text-emerald-400">Claude Sonnet 5</Badge>
        </div>
        <Button size="sm" className="h-7 text-xs bg-emerald-600 hover:bg-emerald-700" onClick={() => setView("new")}>
      ...
      {plans.length === 0 ? (
        <div className="text-center py-10 text-muted-foreground">
          ...
          <p className="text-xs">No rehab plans yet.</p>
          <p className="text-[10px] mt-1">Generate an evidence-based rehabilitation plan for this patient.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {plans.map((p: any) => (
            <div key={p.id} className="flex items-center justify-between p-3 bg-muted/20 border rounded-lg hover:bg-muted/30 cursor-pointer" onClick={() => loadPlan(p.id)}>
              <div className="flex-1 min-w-0">
  ...
  if (view === "new") return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setView("list")}><ArrowLeft className="h-3.5 w-3.5" /></Button>
        <h3 className="text-sm font-semibold">New Rehab Analysis</h3>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2 space-y-1">
          ...
          <Textarea rows={2} className="text-xs" placeholder="e.g. Sharp knee pain on running, started 3 weeks ago after twisting..." value={form.chiefComplaint} onChange={e => setForm(f => ({ ...f, chiefComplaint: e.target.value }))} />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Body Part *</Label>
          <Input className="text-xs h-8" placeholder="e.g. Left knee, Right shoulder" value={form.bodyPart} onChange={e => setForm(f => ({ ...f, bodyPart: e.target.value }))} />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Duration</Label>
          <Input className="text-xs h-8" placeholder="e.g. 3 weeks, 6 months" value={form.duration} onChange={e => setForm(f => ({ ...f, duration: e.target.value }))} />
        </div>
        <div className="space-y-1">
        ...
        <div className="space-y-1">
          ...
          <select className="w-full h-8 text-xs border rounded-md bg-background px-2" value={form.phase} onChange={e => setForm(f => ({ ...f, phase: e.target.value }))}>
            <option value="acute">Acute (0–2 weeks)</option>
        ...
        <div className="space-y-1">
          ...
          <Input className="text-xs h-8" placeholder="e.g. Twisting, fall, overuse" value={form.mechanism} onChange={e => setForm(f => ({ ...f, mechanism: e.target.value }))} />
        </div>
        <div className="space-y-1">
        ...
        <div className="sm:col-span-2 space-y-1">
          ...
          <Input className="text-xs h-8" placeholder="e.g. Stairs, prolonged sitting, overhead activity" value={form.aggravatingFactors} onChange={e => setForm(f => ({ ...f, aggravatingFactors: e.target.value }))} />
        </div>
        <div className="sm:col-span-2 space-y-1">
          <Label className="text-xs">Relieving Factors</Label>
          <Input className="text-xs h-8" placeholder="e.g. Rest, ice, elevation" value={form.relievingFactors} onChange={e => setForm(f => ({ ...f, relievingFactors: e.target.value }))} />
        </div>
        <div className="sm:col-span-2 space-y-1">
          <Label className="text-xs">Relevant History / Comorbidities</Label>
          <Textarea rows={2} className="text-xs" placeholder="Previous injuries, surgeries, diabetes, medications..." value={form.relevantHistory} onChange={e => setForm(f => ({ ...f, relevantHistory: e.target.value }))} />
        </div>
        <div className="sm:col-span-2 space-y-1">
          <Label className="text-xs">Assessment Findings</Label>
          <Textarea rows={2} className="text-xs" placeholder="Body assessment results, ROM, strength tests, special tests..." value={form.assessmentFindings} onChange={e => setForm(f => ({ ...f, assessmentFindings: e.target.value }))} />
        </div>
        <div className="sm:col-span-2 space-y-1">
          <Label className="text-xs">Additional Notes</Label>
          <Textarea rows={2} className="text-xs" placeholder="Anything else relevant..." value={form.additionalNotes} onChange={e => setForm(f => ({ ...f, additionalNotes: e.target.value }))} />
        </div>
      </div>
      <Button className="w-full bg-emerald-600 hover:bg-emerald-700" onClick={handleGenerate} disabled={generating}>
      ...
      <p className="text-[10px] text-muted-foreground text-center">References from PubMed · NICE · Cochrane · BJSM · JOSPT</p>
    </div>
  );
  ...
  if (view === "plan" && plan) return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => { setActivePlan(null); setView("list"); loadPlans(); }}><ArrowLeft className="h-3.5 w-3.5" /></Button>
        <h3 className="text-sm font-semibold">{activePlan.bodyPart} — {activePlan.chiefComplaint}</h3>
        <Badge variant="outline" className="text-[9px] border-emerald-500/30 text-emerald-400">{activePlan.status}</Badge>
      </div>

      {/* Summary Card */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[{label:"Diagnosis",val:plan.diagnosisHypothesis,cols:"sm:col-span-2"},{label:"Severity",val:plan.severity},{label:"Phase",val:plan.phase},{label:"Prognosis",val:plan.prognosis,cols:"sm:col-span-2"},{label:"Return to Activity",val:plan.returnToActivityTimeline,cols:"sm:col-span-2"}].map((item,i)=>(
      ...
      {plan.redFlags?.length > 0 && (
        <div className="p-2.5 bg-red-500/5 border border-red-500/30 rounded-lg">
          <div className="flex items-center gap-1 mb-1"><TriangleAlert className="h-3 w-3 text-red-400" /><p className="text-[9px] font-semibold text-red-400 uppercase">Red Flags to Monitor</p></div>
        ...
        {plan.phases?.map((phase: any, i: number) => (
          <details key={i} className="border rounded-lg" open={i === 0}>
            ...
            <div className="px-3 pb-3 space-y-2.5">
              <div>
                <p className="text-[9px] font-semibold text-muted-foreground uppercase mb-1">Goals</p>
                <ul className="space-y-0.5">{phase.goals?.map((g:string,j:number)=><li key={j} className="text-[10px] flex gap-1"><CheckCircle2 className="h-2.5 w-2.5 text-emerald-400 shrink-0 mt-0.5" />{g}</li>)}</ul>
              </div>
              <div>
                <p className="text-[9px] font-semibold text-muted-foreground uppercase mb-1">BPR Treatments</p>
                <div className="flex flex-wrap gap-1">{phase.bprTreatments?.map((t:string,j:number)=><Badge key={j} variant="outline" className="text-[9px] border-emerald-500/30 text-emerald-400">{t}</Badge>)}</div>
              </div>
              {phase.exercises?.length > 0 && (
                <div>
                  <p className="text-[9px] font-semibold text-muted-foreground uppercase mb-1">Exercises</p>
              ...
              {phase.precautions?.length > 0 && (
                <div>
                  <p className="text-[9px] font-semibold text-amber-400 uppercase mb-1">Precautions</p>
                  <ul className="space-y-0.5">{phase.precautions.map((p:string,j:number)=><li key={j} className="text-[10px] text-amber-300 flex gap-1"><span>⚠</span>{p}</li>)}</ul>
                </div>
              )}
            </div>
          </details>
      ...
      <div className="border rounded-lg">
        <div className="p-3 border-b flex items-center gap-2">
          ...
          <span className="text-[9px] text-muted-foreground">Evidence-based follow-up questions</span>
        </div>
        <div className="max-h-64 overflow-y-auto p-3 space-y-2">
          {(activePlan?.messages || []).map((m: any, i: number) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-lg px-2.5 py-1.5 text-[10px] ${
                m.role === "user" ? "bg-emerald-600 text-white" : "bg-muted/40 text-foreground"
              }`}>{m.content}</div>
            </div>
          ))}
          {chatLoading && <div className="flex justify-start"><div className="bg-muted/40 rounded-lg px-2.5 py-1.5"><Loader2 className="h-3 w-3 animate-spin" /></div></div>}
          <div ref={chatEndRef} />
        </div>
        <div className="p-2 border-t flex gap-2">
          <Input className="text-xs h-8 flex-1" placeholder="Ask about this patient's rehab plan..." value={chatMsg}
            onChange={e => setChatMsg(e.target.value)}
            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleChat(); } }}
          />
          <Button size="sm" className="h-8 w-8 p-0 bg-emerald-600 hover:bg-emerald-700" onClick={handleChat} disabled={chatLoading || !chatMsg.trim()}>
