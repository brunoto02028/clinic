...
// it. SUPERADMIN only, one manual click; the new password only sticks once the
// e-mail was accepted for delivery.
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (studio.type !== "PERSONAL_TRAINER" || !studio.isActive) {
    return NextResponse.json({ error: "Only an active personal-trainer studio has a studio welcome e-mail" }, { status: 400 });
  }

  const owner = await prisma.user.findFirst({
    where: { clinicId: params.id, role: "ADMIN", isActive: true },
    orderBy: { createdAt: "asc" },
    select: { id: true, email: true, firstName: true, password: true },
  });
  if (!owner) return NextResponse.json({ error: "This studio has no active owner" }, { status: 404 });
